...
function ReadingPanel({ book }) {
  ...
  const [mode, setMode] = useLocal('readMode', 'paper');
...
export function Detail() {
  ...
  const relatedFill = related.length < 4
    ? [...related, ...BOOKS.filter(b => b.id !== book.id && !related.some(r => r.id === b.id)).slice(0, 4 - related.length)]
    : related;

  const goRead = () => {
    setTab('read');
    setTimeout(() => readRef.current && window.scrollTo({ top: readRef.current.offsetTop - 80, behavior: 'smooth' }), 60);
  };
    ...
    <div className="container" style={{ paddingTop: 16 }}>
      ...
      <div className="detail-top">
        <div className="detail-cover">
          <div className={'cover-wrap' + (book.featured ? ' featured-trim' : '')}><Cover book={book} foot={catName(book.cat)} /></div>
        </div>
        <div className="detail-head">
          <div className="row" style={{ gap: 8, marginBottom: 12 }}>
            <span className="cat-chip" onClick={() => navigate('category', { id: book.cat })}>{catName(book.cat)}</span>
