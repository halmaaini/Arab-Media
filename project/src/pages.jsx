...
function Library() {
  ...
  const favBooks = favorites.map(id => window.bookById(id)).filter(Boolean);
    ...
    <div className="container" style={{ paddingTop: 24 }}>
      ...
      <div className="cat-grid">
        {window.CATEGORIES.map(c => {
          const sample = window.booksByCat(c.id)[0];
          return (
            <div key={c.id} className="cat-tile" onClick={() => navigate('category', { id: c.id })} style={{ '--cv-bg': sample.palette.bg, '--cv-bg2': sample.palette.bg2, '--cv-accent': sample.palette.accent }}>
              <div className="cat-tile-pattern"></div>
              <div className="cat-tile-body">
                <div className="cat-tile-count tnum">{window.catCount(c.id)} ملخّص</div>
                <h3>{c.name}</h3>
                <p>{c.blurb}</p>
              </div>
            </div>
...
function Category() {
  ...
  const cat = window.catById(route.params.id) || window.CATEGORIES[0];
  const books = window.booksByCat(cat.id);
  return (
  ...
  return (
    <div className="container container-narrow" style={{ paddingTop: 24 }}>
      ...
      <div className="pub-book card">
        <div className="cover-wrap" style={{ width: 84, flex: 'none' }}><Cover book={window.bookById('humain')} /></div>
        <div>
          <span className="eyebrow">من تأليفه — متاح كخلاصة</span>
          <h3 style={{ fontSize: 20, margin: '6px 0' }}>صناعة إنسانٍ ذكيّ</h3>
          <p className="muted" style={{ fontSize: 14.5 }}>{window.bookById('humain').teaser}</p>
...
Object.assign(window, { Library, Categories, Category, About, Publisher, Contact, More, Empty, Field });
