...
function OwnerDash() {
  ...
  const totalListens = window.BOOKS.reduce((s, b) => s + b.listens, 0);
  ...
  const top = [...window.BOOKS].sort((a, b) => b.listens - a.listens).slice(0, 5);
  return (
...
function OwnerContent() {
  ...
  const rows = window.BOOKS.map((b, i) => ({ ...b, status: i % 6 === 5 ? 'draft' : 'published' }))
            ...
            <tbody>
              {rows.map(b => (
                <tr key={b.id}>
                  ...
                  <td className="muted">{window.catName(b.cat)}</td>
...
function OwnerEdit() {
  ...
  const editing = route.params.id ? window.bookById(route.params.id) : null;
        ...
        <div className="stack" style={{ gap: 20 }}>
          ...
          <div className="card edit-card">
            ...
            <div className="field-row">
              ...
              <Field label="التصنيف">
                <select value={f.cat} onChange={e => upd('cat', e.target.value)}>{window.CATEGORIES.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select>
              </Field>
            </div>
            <Field label="الوسوم">
...
function OwnerMessages() {
  const [sel, setSel] = useState(window.MESSAGES[0]);
  const [readIds, setReadIds] = useState(window.MESSAGES.filter(m => m.read).map(m => m.id));
    ...
    <OwnerShell active="owner-settings">
      ...
      <div className="settings-grid">
        ...
        <SettingGroup icon="layers" title="التصنيفات والوسوم" desc="أضف، عدّل، أو رتّب التصنيفات.">
          <div className="row wrap" style={{ gap: 8 }}>{window.CATEGORIES.map(c => <span key={c.id} className="chip chip-removable">{c.name}<button><Icon name="x" /></button></span>)}</div>
        ...
        <SettingGroup icon="star" title="المحتوى المميّز" desc="اختر ملخّص الصدارة.">
          <Field label="ملخّص الصفحة الرئيسية"><select defaultValue="atomic-habits">{window.BOOKS.map(b => <option key={b.id} value={b.id}>{b.title}</option>)}</select></Field>
        </SettingGroup>
      </div>
    </OwnerShell>
  );
...
Object.assign(window, { OwnerLogin, OwnerShell, OwnerDash, OwnerContent, OwnerEdit, OwnerMessages, OwnerSettings, SettingGroup });
