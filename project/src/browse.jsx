    ...
    <div className="stack" style={{ gap: 26 }}>
      <div>
        ...
        <div className="row wrap" style={{ gap: 8 }}>
          {window.CATEGORIES.map(c => <button key={c.id} className={'pill' + (cats.includes(c.id) ? ' active' : '')} onClick={() => toggle(cats, setCats, c.id)}>{c.name}</button>)}
        </div>
      </div>
      <div>
...
function Browse() {
  ...
  const allTags = [...new Set(window.BOOKS.flatMap(b => b.tags))];

  useEffect(() => { setQ(route.params.q || ''); if (route.params.cat) setCats([route.params.cat]); if (route.params.sort) setSort(route.params.sort); }, [route.params.q, route.params.cat, route.params.sort]);

  let results = window.BOOKS.filter(b => {
    if (q) { const hit = b.title.includes(q) || b.author.includes(q) || b.tags.some(t => t.includes(q)) || b.teaser.includes(q) || b.fullText.some(p => p.includes(q)) || b.keyIdeas.some(k => k.t.includes(q)); if (!hit) return false; }
  ...
  results = results.sort((a, b) => sort === 'new' ? b.date.localeCompare(a.date) : sort === 'listens' ? b.listens - a.listens : a.title.localeCompare(b.title, 'ar'));

  const activeFilters = [
    ...cats.map(c => ({ k: 'cat:' + c, l: window.catName(c), clear: () => setCats(cats.filter(x => x !== c)) })),
          ...
          {results.length === 0 ? (
            <div className="empty">
              ...
              <div style={{ marginTop: 36, textAlign: 'start' }}>
                ...
                <Carousel books={[...window.BOOKS].sort((a,b)=>b.listens-a.listens).slice(0,6)} />
              </div>
            </div>
          ) : view === 'grid' ? (
