...
/* ---- Latin-digit formatters (numerals decision: 1,2,3) ---- */
function fmtTime(sec) {
  sec = Math.max(0, Math.floor(sec || 0));
  const m = Math.floor(sec / 60), s = sec % 60;
  return m + ':' + String(s).padStart(2, '0');
}
function fmtDur(sec) { // "18 دقيقة" style label
...
function fmtNumFull(n) { return new Intl.NumberFormat('en-US').format(n); }
...
function Icon({ name, size, stroke, fill, style, className }) {
  const d = PATHS[name];
  const filled = name === 'play' || name === 'pause' || name === 'heart' && fill;
  return (
...
Object.assign(window, { fmtTime, fmtDur, fmtNum, fmtNumFull, fmtDate, useLocal, Icon, Cover, Equalizer, Ring,
