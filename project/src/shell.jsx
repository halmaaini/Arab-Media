...
function SmartSearch({ big }) {
  ...
  const titleHits = ql ? window.BOOKS.filter(b => b.title.includes(q) || b.author.includes(q)).slice(0, 4) : [];
  const catHits = ql ? window.CATEGORIES.filter(c => c.name.includes(q)).slice(0, 3) : [];
  const tagSet = ql ? [...new Set(window.BOOKS.flatMap(b => b.tags).filter(t => t.includes(q)))].slice(0, 4) : [];
  ...
  return (
    <div className={'search-box' + (big ? ' search-big' : '')} ref={ref} style={{ position: 'relative' }}>
      ...
      <input value={q} placeholder="ابحث عن كتاب، مؤلّف، أو موضوع…" onFocus={() => setOpen(true)}
        ...
        <div className="card" style={{ position: 'absolute', top: 'calc(100% + 8px)', insetInlineStart: 0, insetInlineEnd: 0, zIndex: 80, padding: 8, boxShadow: 'var(--shadow-lg)', maxHeight: 420, overflowY: 'auto' }}>
          ...
          {ql && (
            <>
              ...
              {titleHits.map(b => (
                <button key={b.id} className="sug" onClick={() => { setOpen(false); setQ(''); navigate('detail', { id: b.id }); }}>
              ...
              {titleHits.length + catHits.length + tagSet.length === 0 && <div className="sug" style={{ color: 'var(--muted)' }}>لا اقتراحات — اضغط Enter للبحث في النصوص</div>}
            </>
          )}
        </div>
      )}
...
function BottomNav() {
  const { route, navigate, trackId } = useApp();
  const items = [
...
Object.assign(window, { ThemeToggle, SmartSearch, Header, Footer, BottomNav, Toasts });
